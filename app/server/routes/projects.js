...
router.get("/", async (req, res) => {
  try {
    const projects = await store.all("SELECT * FROM project WHERE accounting_unit_id = ? ORDER BY start_date DESC", [req.query.unit]);
    const withTotals = [];
    for (const p of projects) {
      ...
      withTotals.push({ ...p, naklady: totals.naklady, vynosy: totals.vynosy, vysledek: totals.vynosy - totals.naklady });
    }
    res.json(withTotals);
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    store.persist();
    res.status(201).json(await store.get("SELECT * FROM project WHERE id = ?", [id]));
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const existing = await store.get("SELECT * FROM project WHERE id = ? AND accounting_unit_id = ?", [req.params.id, req.user.accountingUnitId]);
    ...
    store.persist();
    res.json(await store.get("SELECT * FROM project WHERE id = ?", [req.params.id]));
  } catch (err) { res.status(400).json({ error: err.message }); }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const docCount = (await store.get("SELECT COUNT(*) AS n FROM document WHERE project_id = ?", [req.params.id])).n;
    const postingCount = (await store.get("SELECT COUNT(*) AS n FROM posting_line WHERE project_id = ?", [req.params.id])).n;
    ...
    await store.run("DELETE FROM project WHERE id = ?", [req.params.id]);
    store.persist();
    res.status(204).end();
  } catch (err) { res.status(400).json({ error: err.message }); }
});
